...
def extract_pattern(file_content, pattern, flatten=False, prefix="", suffix="", offset=0):
...
def main(input_file, pattern, flatten, prefix, suffix, outtag, offset):
    # Read the content of the input file
    with open(input_file, 'r') as file:
        content = file.read()
    
    # Extract the relevant lines, starting from the offset
    extracted_content = extract_pattern(content, pattern, flatten, prefix, suffix, offset)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python extract_pattern.py <input_file> <pattern> [--flatten] [-prefix=<prefix>] [-suffix=<suffix>] [-outtag=<outtag>] [-offset=<offset>]")
        sys.exit(1)
    ...
    main(input_file, pattern, flatten, prefix, suffix, outtag, offset)
