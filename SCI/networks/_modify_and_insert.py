...
def process_lines(file_content, extract_index, insert_index, prefix="", suffix="", offset=0, dropf=0, dropb=0):
    ...
    for line in lines[offset:]:
        ...
        if extract_index < len(values):
            ...
            modified_value = values[extract_index]

            # drop dropf and dropb characters from the front and back of the modified value
            if dropf > 0:
                modified_value = modified_value[dropf:]
            if dropb > 0:
                modified_value = modified_value[:-dropb]

            # Extract the value, modify it with prefix and suffix
            modified_value = f"{prefix}{modified_value}{suffix}"
...
def main(input_file, extract_index, insert_index, prefix="", suffix="", outtag=None, offset=0, dropf=0, dropb=0):
    # Read the content of the input file
    with open(input_file, 'r') as file:
        content = file.read()

    # Process the lines, modifying the value and inserting it at the specified index
    processed_content = process_lines(content, extract_index, insert_index, prefix, suffix, offset, dropf, dropb)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print("Usage: python modify_and_insert.py <input_file> <extract_index> <insert_index> [-dropf=<dropf>] [-dropb=<dropb>] [-prefix=<prefix>] [-suffix=<suffix>] [-outtag=<outtag>] [-offset=<offset>]")
        sys.exit(1)
    ...
    main(input_file, extract_index, insert_index, prefix, suffix, outtag, offset, dropf, dropb)
