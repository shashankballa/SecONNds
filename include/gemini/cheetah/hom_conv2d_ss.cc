...
namespace gemini {
  ...
  Code HomConv2DSS::setUp(const seal::SEALContext &context,
    ...
    std::cout << "HomConv2DSS::setUp() with CONV_USE_CUDA" 
      << "\n"
      << "\t+ " << "isInitCu() = " << std::boolalpha << isInitCu()
      << "\n"
      ;
  ...
  Code HomConv2DSS::conv2DSS(
      ...
      size_t nthreads, bool in_ntt, bool fil_ntt, bool out_ntt) const {
    ...
    auto conv_program = [&](long wid, size_t start, size_t end) {
      for (size_t m = start; m < end; ++m) {
        ...
        size_t used = conv2DOneFilter(image, filters[m], meta, ct_start, 
  ...
  bool HomConv2DSS::isInitCu() const {
    return isInitializedCuda();
  }

  std::string HomConv2DSS::schemeCu() const {
    if (contextCu_) {
      auto _scheme = contextCu_->firstContextData()->parms().scheme();
      switch (_scheme) {
      ...
      return "contextCu_ is not initialized";
    }
  }

  // size_t HomConv2DSS::polyDegreeCu() const {
  //   if (contextCu_) {
  //     return contextCu_->firstContextData()->parms().polyModulusDegree();
  //   } else {
  //     return 0;
  //   }
  // }
  ...
  Code HomConv2DSS::filtersToNttCu(
    ...
    ENSURE_OR_RETURN(isInitCu(), Code::ERR_CONFIG);
  ...
  Code HomConv2DSS::conv2DSSCu(
    ...
    auto imgShare0 = seal2cudaCt1D(img_share0);
    auto imgShare1 = seal2cudaPt1D(img_share1);
    auto filPtCu = seal2cudaPt2D(filters);
    
    MetaCu metaCu;
    metaCu.n_filters = meta.n_filters;
    metaCu.stride = meta.stride;
    metaCu.is_shared_input = meta.is_shared_input;

    auto imageCu = imgShare0;

    // check if evaluatorCu_ is initialized. Print scheme type
    std::cout << "HomConv2DSS::conv2DSSCu() with CONV_USE_CUDA" 
      << "\n"
      << "\t+ " << "isInitCu() = " << std::boolalpha << isInitCu()
      << "\n"
      << "\t+ " << "schemeCu() = " << schemeCu()
    ...
    //     ) {
    //     try {
    //       evaluatorCu_->addPlainInplace(imageCu[i], imgShare1[i]);
    //     } catch (std::logic_error e) {
    //       LOG(WARNING) << "Troy ERROR: " << e.what();
    //       return Code::ERR_INTERNAL;
    //     }
    //   }
    // }
    ...
    // conv2DCu(contextCu_, evaluatorCu_, imgShare0, imgShare1, filPtCu, metaCu, outShare0, 
    //           n_one_channel, in_ntt, fil_ntt, out_ntt);
    // out_share0 = cuda2sealCt1D(outShare0);

    // testing seal2cudaCt1D
    auto _image_share0 = cuda2sealCt1D(imgShare0);
    auto _image_share1 = cuda2sealPt1D(imgShare1);
    auto _filters = cuda2sealPt2D(filPtCu);
    ...
    auto add_program = [&](long wid, size_t start, size_t end) {
      for (size_t i = start; i < end; ++i) {
        try {
          evaluator_->add_plain(_image_share0[i], _image_share1[i], image[i]);
        } catch (std::logic_error e) {
          LOG(WARNING) << "SEAL ERROR: " << e.what();
          return Code::ERR_INTERNAL;
        }
    ...
    if (meta.is_shared_input) {
      image.resize(_image_share0.size(), seal::Ciphertext(tl_pool));
      CHECK_ERR(LaunchWorksHE(tpool, image.size(), add_program), "add");
    } else {
      image = _image_share0;
    }
    ...
    auto conv_program = [&](long wid, size_t start, size_t end) {
      for (size_t m = start; m < end; ++m) {
        seal::Ciphertext *ct_start = &out_share0.at(m * n_one_channel);
        size_t used = conv2DOneFilter(image, _filters[m], meta, ct_start, 
                                      n_one_channel, fil_ntt);
        if (used == (size_t)-1 || used != n_one_channel) {
          return Code::ERR_INTERNAL;
        }
